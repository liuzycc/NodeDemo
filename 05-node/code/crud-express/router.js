...
// var studentFn = require('./student-fs');
var studentFn = require('./student-mongodb');
// var query = require('')
//我们把路由都放在routre容器中
...
router.post('/students/new',function(req,res){
  //添加
  var data = new studentFn(req.body);
...
router.get('/students/edit',function(req,res){
  ...
  //获取当前用户数据
  studentFn.findOne({_id:id},getData);
  function getData(err,arr){
    if(err) return res.send('获取学生信息失败');
    console.log(arr);
...
router.post('/students/edit',function(req,res){
  var params = req.body;
  studentFn.findByIdAndUpdate(params.id.replace(/"/g,''),params,function(err){
...
router.get('/students/delete',function(req,res){
  var id = req.query.id;
  studentFn.findByIdAndRemove(id.replace(/"/g,''),function(msg){
